...
def home():
    ...
    if form.validate_on_submit():
        kepler_id = form.kepler_id.data
        folder_path = os.path.join(app.config['UPLOAD_FOLDER'], '{:09d}'.format(kepler_id))
        os.makedirs(folder_path, exist_ok=True)  # Create folder if not exists
        files = form.files.data  # Grab the files
        for file in files:
            filename = secure_filename(file.filename)
            file.save(os.path.join(folder_path, filename))  # Save each file inside the folder
