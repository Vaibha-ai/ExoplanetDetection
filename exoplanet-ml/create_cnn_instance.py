...
parser.add_argument(
    "--save_model_path",
    type=str,
    help="Path to save the trained model using pickle.")
...
def main(_):
    ...
    #    with open(FLAGS.save_model_path, 'wb') as f:
    #        pickle.dump(estimator, f)
   
    pickle_file_path = 'astronet/test.pkl'
...
# Load the model from the pickle file
    ...
    for attr in dir(loaded_model):
        if not attr.startswith("_"):  # Exclude private attributes
            print(f"{attr}: {getattr(loaded_model, attr)}")
